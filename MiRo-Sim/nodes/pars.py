...
#这个类获得的是关于命令停止后 要多少时间机器人相关的移动才会停止。 平台需要的初始化的参数，都是好关于
#settle down需要的时间。
...
#初始化所有的以上提供的参数 乘上平台给的每秒中的hz频率。得到最后我们需要的一共发送多少次消息的频率。
...
#这里的选择不知道是用来做什么功能的，后来在action选择的地方使用，
class Selection(object):

	def __init__(self):

		#在选择函数中添加一个滞后的参数，和噪声参数
		self.selection_hysteresis = 0.05
		self.selection_noise_mag = 0.01


#这里的是涉及用户感知，在MIRO的外壳上的感应器。
class Lower(object):
...
class Action(object):
	...
	def __init__(self):
		...
		#这里的数值都是概率，动作执行的概率，我估计他们是用的spiking neuronal的概率模型？
...
class CorePars (object):
	...
	def action_demo_flags(self):
		...
		if "L" in flags: print "L disable express through light"
		if "E" in flags: print "E disable express through ears"
		if "Y" in flags: print "Y disable express through eyelids"
		#
		...
		# do config (normally on except where indicated)
		# DOCLINK demo_flags
		# follow DOCLINK (in lib/miro2, lib/miro2_docs) to see all places where flags must be sync'd
		...
		self.flags.EXPRESS_THROUGH_LIGHT = 0		if "L" in flags else 1
		self.flags.EXPRESS_THROUGH_EARS = 0			if "E" in flags else 1
		self.flags.EXPRESS_THROUGH_EYELIDS = 0		if "Y" in flags else 1
		#
		...
		#
		self.flags.DEV_DEBUG_SONAR = 1				if "P" in flags else 0 # NB: normally off!
		self.flags.DEV_DEBUG_DETECTION = 1			if "Q" in flags else 0 # NB: normally off!

		# not accessible from MIROapp
		self.flags.DEV_DEBUG_HALT = 1				if "R" in flags else 0 # NB: normally off!
